...
public class SpectatorCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("spectator")) {
            ...
            if (args.length == 0) {
                player.spigot().setGameMode(GameMode.SPECTATOR);
                player.sendMessage("&7Your gamemode has been updated to &f[Spectator]");
            } else {
                String name = args[0];
                ...
                p.spigot().setGameMode(GameMode.SPECTATOR);

                p.sendMessage(player.getFormattedName() + " &7updated your gamemode to &f[Spectator]");
                player.sendMessage("&7You updated " + p.getFormattedName() + "&7's gamemode to &f[Spectator]");
            }
        }
