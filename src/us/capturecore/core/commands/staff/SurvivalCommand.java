...
public class SurvivalCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("survival")) {
            ...
            if (args.length == 0) {
                player.spigot().setGameMode(GameMode.SURVIVAL);
                player.sendMessage("&7Your gamemode has been updated to &a[Survival]");
            } else {
                String name = args[0];
                ...
                p.spigot().setGameMode(GameMode.SURVIVAL);

                p.sendMessage(player.getFormattedName() + " &7updated your gamemode to &a[Survival]");
                player.sendMessage("&7You updated " + p.getFormattedName() + "&7's gamemode to &a[Survival]");
            }
        }
